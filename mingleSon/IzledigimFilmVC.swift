...
class IzledigimFilmVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func actionButtonClicked(_ sender: Any) {
        ...
        let mediaFolder = storageReference.child("movieMedia") //media klasörüne ulaşıcam - klasörün referansı
                ...
                if error != nil {
                    ...
                    imageReference.downloadURL { url, error in // kullanıcımın kaydettiği verinin hangi urlye kaydedildiğini almak için
                        ...
                        if error == nil {
                            ...
                            let fireStorePost = ["imageUrl" : imageUrl!, "postedBy" : "İzlediğim Film" ,"postBy" : Auth.auth().currentUser!.email! as Any  , "postName" : self.kitapAdıText.text!, "postComment" : self.kitapOzetText.text!, "postNumber" : self.sayfaText.text!, "date" : FieldValue.serverTimestamp(),  ] as [String : Any] // alacağım ve göstereceğim veriler
                            ...
                            firestoreReference = firestoreDatabase.collection("Memories").addDocument(data: fireStorePost, completion: { error in
                                if error != nil {
                                    ...
                                    self.imageView.image = UIImage(named: "addMovie")// kullanıcı tekrar yükleme ekranına geçince o kısmı boş görmesi için
